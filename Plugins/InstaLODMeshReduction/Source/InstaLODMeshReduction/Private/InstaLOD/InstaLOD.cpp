...
#define INSTALOD_ENABLE_RETARGET_BONEPOSE_ANIMATION
			...
			FAnimationRuntime::FillUpComponentSpaceTransformsRefPose(BakePoseData->SkeletalMesh->GetSkeleton(), ComponentSpaceRefPose);
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 2
			FBoneContainer RequiredBones(RequiredBoneIndexArray, UE::Anim::FCurveFilterSettings(), *(BakePoseData->SkeletalMesh));
#else 
			FBoneContainer RequiredBones(RequiredBoneIndexArray, FCurveEvaluationOption(false), *(BakePoseData->SkeletalMesh));
#endif
...
#else
			UE::Anim::BuildPoseFromModel(BakePoseData->BakePoseAnimation->GetDataModel(), Pose, 0.0f, EAnimInterpolationType::Step, RetargetSourceName, Skeleton->GetRefLocalPoses(RetargetSourceName));
#endif
			...
			/// The fnVector3dToVector3f lambda creates a float vector from the double vector.
			...
			for (const uint32 SectionIndex : ValidSectionIndices)
			{
				...
				for (uint32 VertexIndex=Section.BaseVertexIndex; VertexIndex<EndVertexIndex; VertexIndex++)
				{
					...
					if (TotalInfluence > 0)
					{
						...
						{
							const int32 CurrentBoneIndex = Section.BoneMap[BoneIndices[Index]];
					...
					const uint8 WComponent = FMath::Clamp(Vertex.TangentZ.W, 0, TNumericLimits<uint8>::Max());
		...
		for (const uint32 ValidSectionIndex : ValidSectionIndices)
		{
			...
			for (uint32 VertexIndex=Section.BaseVertexIndex; VertexIndex<EndVertexIndex && VertexIndex < (uint32) Vertices.Num(); VertexIndex++, OutVertexIndex++)
			{
				...
				for (uint32 BoneIndex=0u; BoneIndex<MAX_TOTAL_INFLUENCES; BoneIndex++)
				{
					...
					OutBoneData[BoneIndex].BoneInfluence = Vertex.InfluenceWeights[BoneIndex] / (float)TNumericLimits<WeightType>::Max();
				}
			}
		}
	...
	const FSkeletalMeshOptimizationSettings& Settings, bool bCalcLODDistance, const class ITargetPlatform* TargetPlatform)
{
	...
	FSkeletalMeshModel* const SkeletalResource = SkeletalMesh->GetImportedModel();
	FSkeletalMeshLODModel* SourceModel = &SkeletalResource->LODModels[0];
	...
	if (Settings.BaseLOD > 0)
	{
...
#else
		if (Settings.BaseLOD == LODIndex && (!SkeletalMesh->HasMeshDescription(Settings.BaseLOD)))
#endif
		{
		...
		else if (Settings.BaseLOD <= LODIndex && SkeletalMeshResource->LODModels.IsValidIndex(Settings.BaseLOD))
		{
			...
			SourceModel = &SkeletalMeshResource->LODModels[BaseLOD];
		}
		else
		{
			// warn users
	...
	SrcModelBackup.Sections = SourceModel->Sections;
	SrcModelBackup.UserSectionsData = SourceModel->UserSectionsData;
	...
	if (BaseLOD == LODIndex && (SkeletalResource->OriginalReductionSourceMeshData_DEPRECATED.IsValidIndex(BaseLOD) && !SkeletalResource->OriginalReductionSourceMeshData_DEPRECATED[BaseLOD]->IsEmpty()))
	{
		TMap<FString, TArray<FMorphTargetDelta>> TempLODMorphTargetData;
		SkeletalResource->OriginalReductionSourceMeshData_DEPRECATED[BaseLOD]->LoadReductionData(*SourceModel, TempLODMorphTargetData, SkeletalMesh);

		//Restore the section data state (like disabled...)
		RestoreUserSectionsData(SrcModelBackup, *SourceModel);
	...
	{
		FSkeletalMeshLODModel* const Old = LODModels[LODIndex];
		LODModels[LODIndex] = NewModel;

		if (!bReducingSourceModel && Old != nullptr)
		{
	...
	if (SourceModel->NumVertices > 3u)
	{
		...
		TArray<uint32> SkipSections = UEInstaLODSkeletalMeshHelper::GetSkipSectionsForLODModel(SourceModel, LODIndex);
		UEInstaLODSkeletalMeshHelper::SkeletalLODModelToInstaLODMesh(*SourceModel, InstaInputMesh, SkipSections, &BakePoseData, LODIndex);
	...
	if (!bMeshIsReduced)
	{
		FSkeletalMeshLODModel::CopyStructure(NewModel, SourceModel);
											   ...
											   const FSkeletalMeshOptimizationSettings& Settings, const float ElapsedTime)
{
	...
	const FText DetailText = FText::FromString(FString::Printf(TEXT("in %.2fs with %.3f mesh deviation"), ElapsedTime, DisplayMaxDeviation));
	const FText NotificationText = FText::Format(LOCTEXT("LODPluginOptimizeComplete", "Built LOD {0}"), DetailText);
...
public:
	...
	{ 
		...
		for (uint32 Y=0u; Y<Height; Y++)
		{
			for (uint32 X=0u; X<Width; X++)
			{
				...
				OutData[Index].R = FMath::Clamp((uint32)InData[IndexFlippedY].R * TNumericLimits<uint8>::Max() / TNumericLimits<uint16>::Max(), 0u, TNumericLimits<uint8>::Max());
				OutData[Index].G = FMath::Clamp((uint32)InData[IndexFlippedY].G * TNumericLimits<uint8>::Max() / TNumericLimits<uint16>::Max(), 0u, TNumericLimits<uint8>::Max());
				OutData[Index].B = FMath::Clamp((uint32)InData[IndexFlippedY].B * TNumericLimits<uint8>::Max() / TNumericLimits<uint16>::Max(), 0u, TNumericLimits<uint8>::Max());
				OutData[Index].A = TNumericLimits<uint8>::Max();
			}
		}
	}
	...
	{
		const uint32 NumElems = Width * Height;
		const InstaLOD::InstaColorRGBA8* const InData = (InstaLOD::InstaColorRGBA8*)InstaLODTexturePageData;
		OutData.SetNumUninitialized(NumElems);
...
#	define GetFFlattenMaterialPageData(MATERIAL, PAGENAME) (MATERIAL).GetPropertySamples( EFlattenMaterialProperties :: PAGENAME)
	...
	static void ConvertInstaLODMaterialToFlattenMaterial(InstaLOD::IInstaLODMaterial *const InstaMaterial, FFlattenMaterial &OutMaterial, const IInstaLOD::UE_MaterialProxySettings& settings)
	{
		...
		const FColor DefaultColor(255, 255, 255, 255);
		const FColor DefaultNormalColor(127, 127, 127, 255);
															  ...
															  InstaLOD::IInstaLODMaterialData *const MaterialData, InstaLOD::IInstaLODMaterial *const OutInstaMaterial)
	{
		...
		if (TexturePageType == InstaLOD::IInstaLODTexturePage::TypeOpacity)
		{
			...
			float LastAlphaValue = ElementCount > 0u ? OutData[0].A : 0.0f;
			bool bContainsValidAlpha = false;

			for (uint32 ElementIndex = 0u; ElementIndex < ElementCount; ElementIndex++)
			{
				if (LastAlphaValue != OutData[ElementIndex].A)
				{
					bContainsValidAlpha = true;
					break;
				}
			}

			if (!bContainsValidAlpha)
			{
				for (uint32 ElementIndex = 0u; ElementIndex < ElementCount; ElementIndex++)
				{
					OutData->A = OutData->R;
...
struct UEProxyWrapper
{
	...
	bool Execute(InstaLOD::IInstaLODMesh *OutputMesh, const struct FMeshProxySettings& InProxySettings)
	{
		...
		if (MergeOperation != nullptr)
		{
			...
			MeshMergeSettings.Deterministic = CVarProxyDeterministic.GetValueOnAnyThread() > 0;
			...
			if (CVarHLODScreenSizeFactor.GetValueOnAnyThread() > 0.0f)
			{
				...
				OptimizeSettings.Deterministic = CVarProxyDeterministic.GetValueOnAnyThread() > 0;
		...
		else
		{
			...
			RemeshSettings.Deterministic = CVarProxyDeterministic.GetValueOnAnyThread() > 0;
...
{
	ProxyLODCombined(nullptr, &InData, InProxySettings, InputMaterials, InJobGUID);
}

void FInstaLOD::ProxyLOD(const TArray<struct FInstancedMeshMergeData>& InData, const struct FMeshProxySettings& InProxySettings,
	const TArray<struct FFlattenMaterial>& InputMaterials, const FGuid InJobGUID)
{
	ProxyLODCombined(&InData, nullptr, InProxySettings, InputMaterials, InJobGUID);
}

void FInstaLOD::ProxyLODCombined(const TArray<struct FInstancedMeshMergeData>* const InDataInstanceData, const TArray<struct FMeshMergeData>* const InDataMergeData, const struct FMeshProxySettings& InProxySettings, const TArray<struct FFlattenMaterial>& InputMaterials, const FGuid InJobGUID)
{
	check(InDataInstanceData != nullptr || InDataMergeData != nullptr);
	check(!(InDataInstanceData != nullptr && InDataMergeData != nullptr));
	...
	if (InDataInstanceData != nullptr)
	{
		const TArray<struct FInstancedMeshMergeData>& InData = *InDataInstanceData;
		for (int32 DataIndex = 0; DataIndex < InDataInstanceData->Num(); DataIndex++)
		{
			TMap<FName, int32> InMaterialMap;
			TMap<int32, FName> OutMaterialMap;
			...
			if (InData[DataIndex].NewUVs.Num() > 0)
			{
				...
				InstaLOD::InstaVec2F* const OutTexcoords = InstaMesh->GetWedgeTexCoords(0, &NumTexCoordElems);
	...
	{
		const TArray<struct FMeshMergeData>& InData = *InDataMergeData;
		for (int32 DataIndex = 0; DataIndex < InDataInstanceData->Num(); DataIndex++)
		{
			TMap<FName, int32> InMaterialMap;
			TMap<int32, FName> OutMaterialMap;
	...
	const bool bInstaOperationSuccess = InstaOperation.Execute(OutputMesh, InProxySettings);

	if (!bInstaOperationSuccess)
	{
		char InstaLog[8192];
		InstaLOD->GetMessageLog(InstaLog, sizeof(InstaLog), nullptr);
