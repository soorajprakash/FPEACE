...
/*!@mainpage notitle
 * @section welcome_sec Welcome to InstaLOD SDK2024
 ...
 *
 * InstaLOD SDK for Windows has been using Visual Studio 2022 on Windows 10 and requires
 * the installation of the [Visual C++ Redistributables for Visual Studio 2022 (X64)](http://aka.ms/vs/17/release/vc_redist.x64.exe).
 * If you already have installed Visual Studio 2022 or later on your workstation, the installation of the
 ...
 *
 * @section gettingstarted_sec Getting Started with InstaLOD SDK2024
 *
 ...
 *
 * InstaLOD SDK2024 ships with limited sample code. However, a fully documented sample command-line OBJ optimizer is included
 ...
 * InstaLOD SDK2024 ships with a new class that will help you to quickly build renderable mesh representations from your InstaLOD mesh instances.
 ...
 * You're now ready to explore the vast amount of features present in InstaLOD SDK2024. Once you have implemented the conversion between meshes and materials
 * from your app to InstaLOD you will be able to execute most of the operations available in SDK2024 with just a few lines of code.
...
namespace InstaLOD
{
	...
	public:
		 ...
		 * @note The \p mesh MeshType must match this instance.
		 *
	...
	public:
		...
		/**
		 * Gets the vertex position sparse mesh attribute layer.
		 *
		 * @return The mesh data array.
		 */
		...
		/**
		 * Gets the dense mesh polygon attribute layer.
		 *
		...
		/**
		 * Gets the dense mesh wedge attribute layer.
		 *
		 * @return The mesh attribute layer.
		 */
		 ...
		 *
		 * @param [out] outMesh The output mesh buffer for the triangulation.
		 * @return true upon success
		 */
	...
	public:
		...
		enum Type
		{
			...
			TypeOpacity = 4,			   /**< Grayscale opacity map. */
		 ...
		 *  - LA = green and blue undefined, red = luminance
		 *  - L = green, blue and alpha undefined, red = luminance
		 * @param filter the texture filtering type
		 * @param uv the uv coordinate
		 * @param isOpenGL true if sampling with OpenGL UV coordinates
		 * @return the color value as float
		 */
		...
		/**
		 * Swaps the underlying data buffers between both instance.
	 ...
	 *	PNG 8bit, 16bit  (RGB, RGBA, GRAYSCALE, GRAYSCALE ALPHA)
	...
	public:
		...
		/**
		 * Deallocates a texture page handle.
		 * @note To avoid memory leaks, invoke DeallocTexturePageHandle once the handle is not
		 * required anymore. This is typically after passing the handle to another IInstaLODMaterial instance.
		 *
	...
	public:
		...
		bool PersistAllLevels; /**< The flag to switch between saving all levels and only a single level. */
	...
	namespace DistanceFieldStorageType
	{
		enum Type
		{
			...
			DenseStorage,	/**< The distance field is stored in a preallocated dense data structure. Empty cells are still possible, but they consume memory. */
	...
	struct BakeOutputSettings
	{
		...
		bool DisplacementFullRange;												   /**< Enable to output a full range displacement map. This maps the lowest point in the displacement map to 0 and the highest to 1 in the output image. */
	};
	...
	*/
	...
	public:
		...
		*/
		 ...
		 * Performs a CSG operation on \p meshA and \p meshB.
		 * 
		 * To get the complete CSG result, the two output meshes must be merged together.
		 *
		 * @param meshA the A mesh
		 * @param meshB the B mesh
		 * @param outputMeshA the portion of the output mesh that shares the surface of mesh A.
		 * @param outputMeshB the portion of the output mesh that shares the surface of mesh B.
		 * @param settings the settings
		 * @return CSG result
		 */
		virtual ConstructiveSolidGeometryResult Execute(const IInstaLODMesh* meshA, const IInstaLODMesh* meshB, IInstaLODMesh* outputMeshA, IInstaLODMesh* outputMeshB, const ConstructiveSolidGeometrySettings& settings) = 0;
	};
	...
	public:
		 ...
		 * @note InstaLOD will try to communicate with servers at "api.cloud2.instalod.io" via HTTPS.
		 *
...
#define INSTALOD_API_VERSION_MINOR	14
...
#define INSTALOD_API_RELEASEDATE    "2024-10-17"
