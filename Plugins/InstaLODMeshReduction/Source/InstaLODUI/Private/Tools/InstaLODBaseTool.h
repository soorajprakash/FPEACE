...
#include "InstaLODUI/Public/Utilities/InstaLODUtilities.h"
...
public:
	...
	/** 
	 * Checks if the selected LOD index is valid for selection. This function can skip meshes that have less LODs than the selected LOD index.
	 *
	 * @return True if the operation can still be executed.
	 */
	bool IsSelectedMeshesLODSValid();
	...
	/** Returns whether the current settings and selection is going to generate and output multiple assets. */
	inline virtual bool IsGeneratingMultipleAssets() const { return MeshComponents.Num() > 1; }
	...
	/**
	 * Whether the operation is supporting bake meshes.
	 * 
	 * @return True if supported.
	 */
	virtual bool IsBakeMeshSupported() const {
		return false;
	}

	/**
	 * Returns the bake mesh suffix.
	 * 
	 * @return The bake mesh suffix.
	 */
	...
	/* Expand Selection														*/
	/************************************************************************/
	/* Expands the selection in the scene by a certain radius.*/
	...
	UPROPERTY(Config, EditAnywhere, meta = (DisplayName = "Output Path", ToolTip = "The output location of the created assets from the operation. Provided path must be relative to the Contents directory.", DisplayAfter="ResultUsage", EditCondition="ResultUsage == EInstaLODResultUsage::InstaLOD_NewAsset", RelativeToGameContentDir), Category = "Utilities")
	FDirectoryPath NewAssetsOutputDirectory;
	...
	InstaLOD::IInstaLODMesh* InputMesh;
	InstaLOD::IInstaLODMesh* BakeMesh;
	InstaLOD::IInstaLODMesh* OutputMesh;
...
protected:
	/**
	 * Returns a path to save a generated asset from a operation, based on the current settings. This call might block the thread until the user manually selects a path.
	 * 
	 * @param	SuggestedName	A fallback name for the asset being generated.
	 * @returns A unique path in the contents folder to save the asset.
	 */
	template<typename T>
	FString GetAssetOutputPathForType(const FString& SuggestedName) const;
