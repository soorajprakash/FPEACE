...
{
	return NSLOCTEXT(LOCTEXT_NAMESPACE, "ImposterizeToolFriendlyName", "IM");
}

FText UInstaLODImposterizeTool::GetComboBoxItemName() const
{
	return NSLOCTEXT(LOCTEXT_NAMESPACE, "ImposterizeToolComboBoxItemName", "Imposterize");
}

FText UInstaLODImposterizeTool::GetOperationInformation() const
{
	return NSLOCTEXT(LOCTEXT_NAMESPACE, "ImposterizeToolOperationInformation", "The Imposterize operation generates a stand-in mesh for foliage rendering. Foliage meshes are typically not well suited for optimization techniques that would reduce the polygon count as that would ultimately lead to the removal of elements like leaves.\n\nThe operation supports the creation of a wide variety of different imposter types.");
}
...
void UInstaLODImposterizeTool::OnVistaImposter()
{
		...
		{
			const FText Title = NSLOCTEXT(LOCTEXT_NAMESPACE, "OptimizeTool_Title", "Operation warning!");
			FInstaLODModule::MessageDialogOpen(EAppMsgType::Ok, NSLOCTEXT(LOCTEXT_NAMESPACE, "OperationWarning", "Rotating the camera around the forward axis (Roll), causes the result to be inaccurate."), Title);
		}
		...
		const FString SaveObjectPath = GetAssetOutputPathForType<UStaticMesh>(FString::Printf(TEXT("SM_%s_imposter"), *FPackageName::GetShortName(GetDefaultPackageName())));
		...
		{
			OnMeshOperationError(NSLOCTEXT(LOCTEXT_NAMESPACE, "OperationFailed_NoObjectsHeightOrWidth", "The selected objects have no width, or height."));
			return;
		}
...
{
	if (!UInstaLODBaseTool::IsValidJSONObject(JsonObject, TEXT("Imposterize")))
		return false;
	...
	if (SettingsObject->HasField(TEXT("Type")))
	{
		const FString BillboardType = SettingsObject->GetStringField(TEXT("Type"));

		if (BillboardType == TEXT("Flipbook"))
		{
			ImposterizeType = EInstaLODImposterizeType::InstaLOD_Flipbook;
		}
		else if (BillboardType == TEXT("Billboard"))
		{
			ImposterizeType = EInstaLODImposterizeType::InstaLOD_Billboard;
		}
		else if (BillboardType == TEXT("AABB"))
		{
			ImposterizeType = EInstaLODImposterizeType::InstaLOD_AABB;
		}
		else if (BillboardType == TEXT("HybridBillboardCloud"))
		{
	...
	if (SettingsObject->HasField(TEXT("QuadXYCount")))
	{
		XYAxisQuads_Billboard = SettingsObject->GetIntegerField(TEXT("QuadXYCount"));
	}
	if (SettingsObject->HasField(TEXT("QuadXZCount")))
	{
		XZAxisQuads_Billboard = SettingsObject->GetIntegerField(TEXT("QuadXZCount"));
	}
	if (SettingsObject->HasField(TEXT("QuadYZCount")))
	{
		YZAxisQuads_Billboard = SettingsObject->GetIntegerField(TEXT("QuadYZCount"));
	}
	if (SettingsObject->HasField(TEXT("CloudFaceCount")))
	{
		MaximumFaceCount = SettingsObject->GetIntegerField(TEXT("CloudFaceCount"));
	}
	if (SettingsObject->HasField(TEXT("CloudPolyFaceFactor")))
	{
		HybridMeshFaceFactor = (float) SettingsObject->GetNumberField(TEXT("CloudPolyFaceFactor")) * 100.0f;
	}
	if (SettingsObject->HasField(TEXT("EnableCloudQuadTwoSided")))
	{
		bTwoSidedQuads_Hybrid = SettingsObject->GetBoolField(TEXT("EnableCloudQuadTwoSided"));
	}
	if (SettingsObject->HasField(TEXT("EnableCloudQuadXY")))
	{
		bXYAxisQuads = SettingsObject->GetBoolField(TEXT("EnableCloudQuadXY"));
	}
	if (SettingsObject->HasField(TEXT("EnableCloudQuadXZ")))
	{
		bXZAxisQuads = SettingsObject->GetBoolField(TEXT("EnableCloudQuadXZ"));
	}
	if (SettingsObject->HasField(TEXT("EnableCloudQuadYZ")))
	{
		bYZAxisQuads = SettingsObject->GetBoolField(TEXT("EnableCloudQuadYZ"));
	}
	...
	if (SettingsObject->HasField(TEXT("EnableQuadTwoSided")))
	{
		bTwoSidedQuads_Billboard = SettingsObject->GetBoolField(TEXT("EnableQuadTwoSided"));
	}
	if (SettingsObject->HasField(TEXT("FlipbookFramesPerAxis")))
	{
		FlipbookFramesPerAxis = SettingsObject->GetIntegerField(TEXT("FlipbookFramesPerAxis"));
	}
	if (SettingsObject->HasField(TEXT("CloudNormalConformity")))
	{
		CloudNormalConformity = (float)SettingsObject->GetNumberField(TEXT("CloudNormalConformity"));
	}
	if (SettingsObject->HasField(TEXT("CloudNormal")))
	{
		const FString CloudNormalString = SettingsObject->GetStringField(TEXT("CloudNormal"));
...
bool UInstaLODImposterizeTool::CanAppendMeshToInput(FInstaLODMeshComponent& Component, InstaLOD::IInstaLODMesh *Mesh, TArray<UMaterialInterface*>* UniqueMaterials)
{
	...
	{
		InstaLOD::IInstaLODMesh* TempMesh = GetInstaLODInterface()->AllocInstaLODMesh();
		InstaLOD::IInstaLODMesh* SubMesh = GetInstaLODInterface()->AllocInstaLODMesh();
		...
		if (AuxMesh == nullptr)
		{
			AuxMesh = GetInstaLODInterface()->AllocInstaLODMesh();
		}
...
bool UInstaLODImposterizeTool::IsMeshOperationExecutable(FText* OutErrorText) const
{
	...
	if (ImposterizeType == EInstaLODImposterizeType::InstaLOD_Billboard)
	{
		if (XYAxisQuads_Billboard == 0 && XZAxisQuads_Billboard == 0 && YZAxisQuads_Billboard == 0)
		{
			...
			{
				*OutErrorText = NSLOCTEXT(LOCTEXT_NAMESPACE, "NoBillboards", "You need to enable at least one billboard quad axis to create a billboard imposter.");
			}
			return false;
		}
	}
	...
	if (ImposterizeType == EInstaLODImposterizeType::InstaLOD_Vista)
	{
		...
		if ((SelectedStaticMeshActors.Num() < 1 && InstaLODCameraComponents.Num() != 1) || InstaLODCameraComponents.IsEmpty() || SelectedStaticMeshActors.IsEmpty())
		{
			if (OutErrorText != nullptr)
			{
				*OutErrorText = NSLOCTEXT(LOCTEXT_NAMESPACE, "InvalidSelection", "Please select a camera, and at least one actor.");
			}
			return false;
		}
	}
