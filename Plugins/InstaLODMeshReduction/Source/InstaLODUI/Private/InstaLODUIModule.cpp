...
void FInstaLODUIModule::StartupModule()
{
	...
	if (ToolBar != nullptr)
	{
		...
		FToolMenuEntry InstaLODButton = FToolMenuEntry::InitComboButton(
			...
			NSLOCTEXT(LOCTEXT_NAMESPACE, "InstaLOD", "InstaLOD"),
			NSLOCTEXT(LOCTEXT_NAMESPACE, "InstaLODToolTip", "InstaLOD Mesh Optimization"),
...
TSharedRef<class SDockTab> FInstaLODUIModule::OnSpawnInstaLODTab(const class FSpawnTabArgs& SpawnTabArgs)
{
		...
		ReductionModule.GetMeshMergingInterface() != InstaLODModule.GetMeshMergingInterface())
	{
		...
		[
		 ...
		 [
		  SNew(STextBlock)
		  .Text(NSLOCTEXT(LOCTEXT_NAMESPACE, "BadSetup_Title", "InstaLOD is not setup properly!"))
		  .AutoWrapText(true)
		 ...
		 [
		  ...
		  .Text(NSLOCTEXT(LOCTEXT_NAMESPACE, "BadSetup_Message",
							...
							"\n\n4. Restart the engine. After restarting the engine, InstaLOD is ready to use."
			 ))
