...
void FInstaLODBaseToolCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (RemeshTool != nullptr)
	{
		...
		if(RemeshTool->GetActiveSetting() == EInstaLODRemeshType::InstaLOD_ScreenSizeInPixels)
		{
	...
	UInstaLODMaterialMergeTool* const MaterialMergeTool = Cast<UInstaLODMaterialMergeTool>(ToolInstance);

	if (MaterialMergeTool != nullptr)
	{
		// setup a delegate that is called when the type property changes
		if (!RefreshDetailsDelegate.IsBound())
		{
	...
	UInstaLODImposterizeTool* const ImposterizeTool = Cast<UInstaLODImposterizeTool>(ToolInstance);
	...
	UInstaLODOcclusionCullTool* const OcclusionCullTool = Cast<UInstaLODOcclusionCullTool>(ToolInstance);

	if (OcclusionCullTool != nullptr)
	{
		// setup a delegate that is called when the type property changes
		if (!RefreshDetailsDelegate.IsBound())
		{
...
{
	if (ToolInstance != nullptr && ToolInstance->GetInstaLODWindow())
...
void FInstaLODBaseToolCustomization::OptimizeToolSettingsCheckBoxClicked(const ECheckBoxState NewCheckedState, int32 CheckBoxIndex)
{
	...
	{
		OptimizeTool->SetActiveSetting((EInstaLODOptimizeType) CheckBoxIndex);
	}
}
...
ECheckBoxState FInstaLODBaseToolCustomization::IsOptimizeToolSettingsCheckBoxSelected(int32 CheckBoxIndex) const
{
	...
	return (OptimizeTool->GetActiveSetting() == (EInstaLODOptimizeType) CheckBoxIndex) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
...
bool FInstaLODBaseToolCustomization::IsOptimizeToolSettingEnabled(int32 CheckBoxIndex) const
{
	...
	return (OptimizeTool->GetActiveSetting() == (EInstaLODOptimizeType) CheckBoxIndex);
}
...
void FInstaLODBaseToolCustomization::RemeshToolSettingsCheckBoxClicked(const ECheckBoxState NewCheckedState, int32 CheckBoxIndex)
{
	...
	if (NewCheckedState == ECheckBoxState::Checked)
	{
		RemeshTool->SetActiveSetting((EInstaLODRemeshType) CheckBoxIndex);
...
ECheckBoxState FInstaLODBaseToolCustomization::IsRemeshToolSettingsCheckBoxSelected(int32 CheckBoxIndex) const
{
	...
	return (RemeshTool->GetActiveSetting() == (EInstaLODRemeshType) CheckBoxIndex) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
...
bool FInstaLODBaseToolCustomization::IsRemeshToolSettingEnabled(int32 CheckBoxIndex) const
{
	...
	return (RemeshTool->GetActiveSetting() == (EInstaLODRemeshType) CheckBoxIndex);
}
