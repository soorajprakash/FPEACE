	...
	/************************************************************************/
	{
		...
		if (Settings->bAuthorized)
		{
			...
			for (TFieldIterator<UFunction> FuncIt(SettingsClass); FuncIt; ++FuncIt)
			{
				...
				if (Function->HasAnyFunctionFlags(FUNC_Exec) && (Function->NumParms == 0) && Function->GetMetaData(FName(TEXT("Category"))).Equals(TEXT("Deauthorize")))
				{
		...
		{
			SInstaLODWindow* const InstaLODWindowPtr = Settings->GetInstaLODWindow();
		...
		{
			WarningText = NSLOCTEXT(LOCTEXT_NAMESPACE, "NodeLockedWarning", "This workstation has been authorized with a node-locked license. If you want to deauthorize this workstation, please contact your account manager at InstaLOD.");
