...
void FInstaLODOptimizeToolCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TSharedPtr<IPropertyHandle> BakePoseHandle = DetailBuilder.GetProperty(FName(TEXT("BakePose")), UInstaLODOptimizeTool::StaticClass());
	BakePoseHandle->MarkHiddenByCustomization();

	const FText BakePoseLabel = FText::FromString(TEXT("Bake Pose"));
	...
	auto fnFilterValidBakePoses = [this](const FAssetData& AssetData) -> bool
	{
		...
		const FString SkeletonName = AssetData.GetTagValueRef<FString>(TEXT("TargetSkeleton"));
	...
	[
		...
		.Text(NSLOCTEXT(LOCTEXT_NAMESPACE, "BakePoseSkeletal", "Bake Pose"))
	]
	.ValueContent()
	[
	...
	IDetailCategoryBuilder& CVarCategory = DetailLayoutBuilder->EditCategory("Mesh Editor Reduction Settings", FText::FromString(TEXT("Mesh Editor Reduction Settings")), ECategoryPriority::Default);
	...
	{
		{ TEXT("InstaLOD.ForceOptimizerWeights"), TEXT("Vertex Colors As Optimizer Weights"), TEXT("Mesh Editor: Vertex Colors As Optimizer Weights")},
		{ TEXT("InstaLOD.LockBoundaries"), TEXT("Lock Boundaries"), TEXT("Mesh Editor: Prevents optimization of vertices on the mesh boundary. This is useful if the mesh contains holes that need to be preserved as is.")},
		{ TEXT("InstaLOD.LockSplits"), TEXT("Lock Splits"), TEXT("Mesh Editor: Prevents optimization of split vertices. This is useful if UV and normal splits need to be preserved as is.")},
		{ TEXT("InstaLOD.OptimizeDeterministic"), TEXT("Deterministic"), TEXT("Mesh Editor: Makes the algorithm deterministic at the cost of speed.")}
	};
	...
	for (const CVarInfo& Info : CVarInfos)
	{
		...
		CVarCategory.AddCustomRow(FText::FromString(TEXT("Mesh Editor Reduction Settings")))
		.NameContent()
		[
			SNew(STextBlock)
