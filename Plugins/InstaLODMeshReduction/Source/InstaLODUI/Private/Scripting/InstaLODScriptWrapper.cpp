...
namespace InstaLODScriptUtilities
{
	 ...
	 */
	static bool GenerateCloudPolygonalMesh(InstaLOD::IInstaLODMesh* const Mesh, InstaLOD::IInstaLODMesh* const CloudPolyMesh, TArray<UMaterialInterface*>& UniqueMaterials, const FString& HybridBillboardCloudPolySuffix, IInstaLOD* const InstaLODInterface)
	{
		check(Mesh != nullptr);
		...
		{
			InstaLOD::IInstaLODMesh* const TempMesh = InstaLODInterface->AllocInstaLODMesh();
			InstaLOD::IInstaLODMesh* const SubMesh = InstaLODInterface->AllocInstaLODMesh();
...
public:
	...
	{ 
		...
		FScopedSlowTask Task(Entries.Num(), NSLOCTEXT(LOCTEXT_NAMESPACE, "ScriptStart", "InstaLOD Script Operation in progress"));

		for (UObject* const Entry : Entries)
		{
			Task.EnterProgressFrame(1.0f, NSLOCTEXT(LOCTEXT_NAMESPACE, "ScriptProgress", "InstaLOD Script Operation in progress"));
...
UInstaLODScriptResult* UInstaLODScriptWrapper::RemeshAssets(const TArray<UObject*> Entries, int32 BaseLODIndex, UInstaLODRemeshSettings* const RemeshSettings, UInstaLODResultSettings* const ResultSettings, UInstaLODBakeOutputSettings* const MaterialSettings)
{
	...
	Task.EnterProgressFrame(1, NSLOCTEXT(LOCTEXT_NAMESPACE, "ScriptProgress", "InstaLOD Script Operation in progress"));
