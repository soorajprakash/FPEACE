...
#include "InstaLODUI/Public/Scripting/Settings/InstaLODResultSettings.h"
...
class FInstaLODMeshMergeUtilities : public IMeshMergeUtilities
{
public:
	void CreateMaterialData(IInstaLOD* InstaLOD, TArray<InstaLODMergeData>& MergeData, InstaLOD::IInstaLODMaterialData* MaterialData, const struct FMaterialProxySettings& InMaterialProxySettings, TArray<UMaterialInterface*>& OutMaterials) const
	{
		...
		const int32 MergeDataCount = MergeData.Num();
		if (MergeDataCount == 0)
		{
		...
		FScopedSlowTask SlowTask((float)MergeDataCount, (NSLOCTEXT(LOCTEXT_NAMESPACE, "CreateMaterialData", "Creating Material Data")));
		SlowTask.MakeDialog();
		...
		/**
		 * The FFullBakeOptions struct contains information
		 * about whether a material requires a full bake and
		 * whether the UVs require to be recalculated.
		 */
		struct FFullBakeOptions
		{
			FFullBakeOptions() = delete;

			FFullBakeOptions(const bool bInMaterialRequiresBake, const bool bInUVsRequireRecalculation) :
			bMaterialRequiresBake(bInMaterialRequiresBake),
			bUVsRequireRecalculation(bInUVsRequireRecalculation)
			{
			}

			bool bMaterialRequiresBake;		/**< Whether the material requires a full bake. */
			bool bUVsRequireRecalculation;	/**< Whether the UVs require to be recalculated. */
		};

		TArray<FVector2D> UniqueUVs;
		for (int32 MergeDataIndex = 0; MergeDataIndex < MergeDataCount; ++MergeDataIndex)
		{
			SlowTask.EnterProgressFrame(/*ExpectedWorkThisFrame:*/ 1.0f, FText::Format(NSLOCTEXT(LOCTEXT_NAMESPACE, "CreateMaterialData_FlatteningMaterialData", "Flattening Material: {0}/{1}"), MergeDataIndex, MergeDataCount));
			SlowTask.MakeDialog();

			const InstaLODMergeData& InstaLODMergeData = MergeData[MergeDataIndex];
			...
			TArray<FFullBakeOptions> MaterialRequiresFullBake;

			// Check if texture coordinates exceed the normalized UV Space.
			TArray<FVector2f>& UVs = RawMesh.WedgeTexCoords[0];
			const bool bTextureCoordinatesOutOfBounds = UVs.FindByPredicate([](const FVector2f& Coordinates) { return Coordinates.X > 1.0f || Coordinates.X < 0.0f || Coordinates.Y > 1.0f || Coordinates.Y < 0.0f; }) != nullptr;
			...
			for (const FSectionInfo& Section : Sections)
			{
				...
				MaterialRequiresFullBake.Add(FFullBakeOptions(bTextureCoordinatesOutOfBounds || NumTexCoords > 1 || bUseVertexData, bTextureCoordinatesOutOfBounds));
			}

			// update full bake settings
			for (const FFullBakeOptions& BakeOptions : MaterialRequiresFullBake)
			{
				if (!BakeOptions.bMaterialRequiresBake)
					continue;
				...
				// generate new non overlapping UVs if required
				if (BakeOptions.bUVsRequireRecalculation)
				{
					UniqueUVs.SetNum(0, /*bAllowShrinking:*/ false);

					// NOTE: "GenerateUV()" does not work correctly on macOS on version 5.4 for meshes with more than 2 UV channels as Unreal Engine crashes at a memory free() call. 
					// Using the deprecated "GenerateUniqueUVsForStaticMesh()" on macOS until Unreal provides a fix.

#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 4 || PLATFORM_MAC
					FStaticMeshOperations::GenerateUniqueUVsForStaticMesh(MeshDescription, Options->TextureSize.GetMax(), /*bMergeIdenticalMaterials:*/ false, UniqueUVs);
#else
					FStaticMeshOperations::FGenerateUVOptions UVOptions;
					UVOptions.TextureResolution = Options->TextureSize.GetMax();
					UVOptions.bMergeTrianglesWithIdenticalAttributes = false;
					UVOptions.UVMethod = FStaticMeshOperations::EGenerateUVMethod::UVAtlas;
					
					FStaticMeshOperations::GenerateUV(MeshDescription, UVOptions, UniqueUVs);
#endif
					MeshSetting.CustomTextureCoordinates = UniqueUVs;
			...
			for (const FSectionInfo& Section : Sections)
			{
				...
				MaterialSetting.BlendMode = Section.Material->GetBlendMode();
	...
	{
		UMaterialOptions* const MaterialOptions = DuplicateObject(GetMutableDefault<UMaterialOptions>(), GetTransientPackage());
	...
	void PopulatePropertyEntry(const FMaterialProxySettings& MaterialSettings, EMaterialProperty MaterialProperty, FPropertyEntry& InOutPropertyEntry) const
	{
		...
		default:
			UE_LOG(LogInstaLOD, Error, TEXT("Unsupported TextureSizingType value."));
...
void UInstaLODUtilities::AppendMeshComponentToInstaLODMesh(IInstaLOD* InstaLOD, TSharedPtr<FInstaLODMeshComponent> MeshComponent, InstaLOD::IInstaLODMesh* OutInstaLODMesh, int32 BaseLODIndex)
{
	...
	if (InstaLODMesh != nullptr)
	{
		OutInstaLODMesh->AppendMesh(InstaLODMesh);
...
UMaterialInstanceConstant* UInstaLODUtilities::CreateFlattenMaterialInstance(const FFlattenMaterial& FlattenMaterial, const FMaterialProxySettings& InMaterialProxySettings, const FString& SaveObjectPath, TArray<UObject*>& OutAssetsToSync, bool bIsFlipbookMaterial)
{
	...
	const FString InstaLODMaterialAsset = TEXT("/InstaLODMeshReduction/InstaLODFlattenMaterial.InstaLODFlattenMaterial");
#else
	const FString InstaLODMaterialAsset = TEXT("/InstaLODMeshReduction/InstaLODFlattenMaterial_5_2.InstaLODFlattenMaterial");
#endif

	const FString BaseMaterialAsset = bIsFlipbookMaterial ? TEXT("/InstaLODMeshReduction/InstaLODFlattenFlipbookMaterial.InstaLODFlattenFlipbookMaterial") : InstaLODMaterialAsset;
...
void UInstaLODUtilities::RetrieveMesh(UStaticMeshComponent *const StaticMeshComponent, int32 LODIndex, FMeshDescription& OutMeshDescription, bool bPropagateVertexColours, bool bInWorldSpace)
{
	...
	if(BuildSettings.bRecomputeNormals)
	{
		...
		if (TriangleNormals.IsValid())
		{
			const FTriangleArray& Triangles = OutMeshDescription.Triangles();

			// make sure valid polygon normals are set
			if (TriangleNormals.GetNumElements() != Triangles.Num())
			{
...
void UInstaLODUtilities::InsertLODToStaticMesh(IInstaLOD* InstaLOD, UStaticMesh* StaticMesh, InstaLOD::IInstaLODMesh* InstaLODMesh, int32 TargetLODIndex, UMaterialInterface* NewMaterial)
{
	...
	// Invalid or no TargetLODIndex Specified, so we assume the user wants to append to the existing LODs.
...
void UInstaLODUtilities::InsertLODToSkeletalMesh(IInstaLOD* InstaLOD, USkeletalMesh* SkeletalMesh, InstaLOD::IInstaLODMesh* InstaLODMesh, int32 TargetLODIndex, UMaterialInterface* NewMaterial)
{
	...
	{
		SkeletalMesh->GetLODInfoArray()[TargetLODIndex].LODMaterialMap.Reset();
	}
	...
	// fix material slot names and validate all LOD material indices.
	if (NewMaterial != nullptr)
	{
		if (IMeshUtilities* MeshUtilities = FModuleManager::Get().LoadModulePtr<IMeshUtilities>("MeshUtilities"))
		{
			MeshUtilities->FixupMaterialSlotNames(SkeletalMesh);
		}

		SkeletalMesh->GetLODInfoArray()[TargetLODIndex].LODMaterialMap.Reset();
		SkeletalMesh->GetLODInfoArray()[TargetLODIndex].LODMaterialMap.Add(NewMaterialIndex);
...
InstaLOD::IInstaLODMesh* UInstaLODUtilities::CreatePlane(class IInstaLOD* InstaLOD, const FVector2d Extents)
{
	...
	const uint32 PositionCount = 4u;
	const uint32 WedgeCount = 6u;
	const uint32 FaceCount = 2u;
	...
	InstaVec3F* const WedgeTangents = InstaMesh->GetWedgeTangents(nullptr);
	InstaVec3F* const WedgeBinormals = InstaMesh->GetWedgeBinormals(nullptr);
	...
	WedgeTangents[0] = InstaVec3F(0.0f, 1.0f, 0.0f);
	WedgeTangents[1] = InstaVec3F(0.0f, 1.0f, 0.0f);
	WedgeTangents[2] = InstaVec3F(0.0f, 1.0f, 0.0f);
	WedgeTangents[3] = InstaVec3F(0.0f, 1.0f, 0.0f);
	WedgeTangents[4] = InstaVec3F(0.0f, 1.0f, 0.0f);
	WedgeTangents[5] = InstaVec3F(0.0f, 1.0f, 0.0f);

	WedgeBinormals[0] = InstaVec3F(-1.0f, 0.0f, 0.0f);
	WedgeBinormals[1] = InstaVec3F(-1.0f, 0.0f, 0.0f);
	WedgeBinormals[2] = InstaVec3F(-1.0f, 0.0f, 0.0f);
	WedgeBinormals[3] = InstaVec3F(-1.0f, 0.0f, 0.0f);
	WedgeBinormals[4] = InstaVec3F(-1.0f, 0.0f, 0.0f);
	WedgeBinormals[5] = InstaVec3F(-1.0f, 0.0f, 0.0f);
