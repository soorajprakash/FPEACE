...
) const
{
...

	{
		...
		static auto PropagateAlphaCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.PostProcessing.PropagateAlpha"))->AsVariableBool();
		check(PropagateAlphaCVar);

		const bool bEnableAlphaUpscaling = (CVarNGXEnableAlphaUpscaling.GetValueOnRenderThread()) &&
										   (PropagateAlphaCVar->GetValueOnRenderThread() != false);
